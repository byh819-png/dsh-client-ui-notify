...
export const NOTIFY_METHODS = ['builtin', 'tts', 'custom'] as const
...
export const AUDIO_EXTENSION_MEDIA_TYPES: Record<string, string> = {
  wav: 'audio/wav',
  mp3: 'audio/mpeg',
  ogg: 'audio/ogg',
  mp4: 'audio/mp4',
...
const AUDIO_MEDIA_TYPE_EXTENSIONS: Record<string, string> = Object.fromEntries(
  Object.entries(AUDIO_EXTENSION_MEDIA_TYPES).map(([extension, mediaType]) => [mediaType, extension]),
)
 ...
 */
export function audioExtensionOfMediaType(mediaType: string, fileName?: string): string | undefined {
 ...
 */
export function audioMediaTypeOfExtension(extension: string): string | undefined {
 ...
 */
export function isNotifyMethod(value: unknown): value is NotifyMethod {
